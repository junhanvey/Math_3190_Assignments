...
geom_point()+
labs(x = 'Log_10 Of Brain Weight', y = 'Log_10 of Body Weight')
```
...
ggplot(msleep, aes(x <- conservation, fill = conservation)) +
  ...
  theme_minimal() +
    labs(x = "Conservation", y = "Count") +
  scale_fill_manual(values = c("cornflowerblue", "lightpink","lightyellow",
                               "palegreen","lavender", "plum1", "cadetblue3"))
...
```{r, eval = T, echo = T}
ggplot(msleep, aes(x = awake)) +
  geom_histogram(bins = 10, color = 'black', fill = "aquamarine") 

ggplot(msleep, aes (x = awake)) +
  geom_boxplot(color = 'hotpink', fill = 'lightpink') +
  theme_minimal()
...
```{r, echo = T, eval = T}
college = read_csv("CollegeDistance.csv", show_col_types = F)
```
...
``` {r, echo = T, eval = T}
ggplot(college, aes(x = gender, fill = gender)) +
  geom_bar(color = 'black') +
  labs(x = "Gender", y = "Frequency") +
  scale_fill_manual(values = c('hotpink','cornflowerblue'))
...
``` {r, echo = T, eval = T}
ggplot(college, aes(x = "", y = ethnicity, fill = ethnicity)) +
...
ggplot(college, aes(x = income, fill = ethnicity)) +
  ...
  scale_fill_manual(values = cbPalette, labels = c("African American",
                                                   "Hispanic",
                                                   "Other")) +
  labs(x = "Income Level", y = "Proportion", fill = "Ethnicity") 
...
ggplot(college, aes(x = income, fill = ethnicity)) +
  geom_bar(color = "black", position = "fill") + 
  scale_fill_brewer(palette = 'Set3', labels = c("African American",
                                                   "Hispanic",
                                                   "Other")) +
  labs(x = "Income Level", y = "Proportion", fill = "Ethnicity") 
...
``` {r, echo = T, eval = T}
ggplot(college, aes(x = unemp, y = tuition, color = region)) +
  geom_point() +
  labs(x = "Unemployment Rate", y = "Average 4 Year College Tuition (in thousands)", title = "College Tuition and Employment") +
  theme(plot.title = element_text(size = 14, face = "bold"),
        axis.title = element_text(size = 14)) +
  scale_y_continuous(name = "Average 4 Year College Tuition (in thousands)"
                   , breaks = seq(0, 2, 0.25))
...
ggplot(college, aes(x = distance)) +
  geom_histogram(color = "black", fill = "mediumpurple3") +
  labs(x = "Distance in Miles", y = 'Frequency', title = "Distance from
       College") +
  theme_minimal()
...
``` {r, echo = T, eval = T}
ggplot(college, aes(x = score, y = income, fill = income)) +
  geom_boxplot() +
  labs(x = 'Score', y = 'Income', fill = "income", title = 
         "Boxplots of Score by Income") +
  scale_fill_manual(values = c("lightgoldenrod1","palegreen1")) +
  theme_bw()
```
...
```{r, echo = T, eval = T}
al = read_csv('airlines.csv')
al$airline <- as.factor(al$airline)
str(al)
...
```{r, echo = T, eval = T}
ggplot(al, aes(x = year, y = cost)) +
 geom_point() + facet_grid(cols = vars(Airline), labeller = label_both) + geom_line()
```
