...
### This code chunk creates the foundation for the app 
...
shinyApp(ui = ui, server = server)
```
### This code chunk added the k-means clustering title 
...
shinyApp(ui = ui, server = server)
```
### This code creates the side menu where users can input the  X & Y variables and the cluster count
...
shinyApp(ui = ui, server = server)
```
### This chunk just specifies that the output of the user inputs is a plot and begins defining the server
### Step 04: Output plot
...
shinyApp(ui = ui, server = server)
```
### This step creates an attribute of output, aka the plot of sepal length and sepal width
...
shinyApp(ui = ui, server = server)
```
### The axes on the plot now react to the user inputs instead of just being the plot of the two variables in the last chunk
### Step 06: K means
...
shinyApp(ui = ui, server = server)
```
### The points on the scatter plot are now colored by cluster
...
shinyApp(ui = ui, server = server)
```
### Adds an X to the center of each cluster
### Step 08: Save Kmeans
...
shinyApp(ui = ui, server = server)
```
### Fills in points on scatter plot, 
